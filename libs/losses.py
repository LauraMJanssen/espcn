...
class VGGLossNoActivation(object):
    ...
    def mse_content_loss(self, y_true, y_pred):
        return 1e-5 * K.mean(
            K.square(self.model(self.preprocess_vgg(y_true)) - self.model(self.preprocess_vgg(y_pred))), None)

    def euclidean_content_loss(self, y_true, y_pred):
        return K.sqrt(K.sum(K.square(self.model(self.preprocess_vgg(y_true)) - self.model(self.preprocess_vgg(y_pred))),
                            axis=None))

    def plus_content_loss(self, y_true, y_pred):
        return (1e-5 * K.mean(
            K.square(self.model(self.preprocess_vgg(y_true)) - self.model(self.preprocess_vgg(y_pred))), None) + K.mean(
            K.square(y_pred - y_true), axis=None))
...
class VGGLoss(object):
    ...
    def plus_content_loss(self, y_true, y_pred):
        return (1. * K.mean(
            K.square(self.model(self.preprocess_vgg(y_true)) - self.model(self.preprocess_vgg(y_pred)))) + K.mean(
            K.square(y_pred - y_true), axis=None))
