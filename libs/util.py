...
class DataLoader(Sequence):
    def __init__(self, datapath, batch_size, height_hr, width_hr, scale, crops_per_image, media_type, channels=3,
                 colorspace='RGB'):
        """
...
def plot_test_images(model, loader, datapath_test, test_output, epoch, name='SRCNN', channels=3, colorspace='RGB'):
    try:
        ...
        test_images = [os.path.join(datapath_test, f) for f in os.listdir(datapath_test) if
                       any(filetype in f.lower() for filetype in ['jpeg', 'mp4', '264', 'png', 'jpg'])]
        ...
        if channels == 1:
            ...
            if (colorspace == 'YCbCr'):
                imgs_lr = [cv2.cvtColor(loader.unscale_lr_imgs(img).astype(np.uint8), cv2.COLOR_YCrCb2BGR) for img in
                           imgs_lr]
                imgs_hr = [cv2.cvtColor(loader.unscale_hr_imgs(img).astype(np.uint8), cv2.COLOR_YCrCb2BGR) for img in
                           imgs_hr]
                imgs_sr = [cv2.cvtColor(loader.unscale_hr_imgs(img).astype(np.uint8), cv2.COLOR_YCrCb2BGR) for img in
                           imgs_sr]
